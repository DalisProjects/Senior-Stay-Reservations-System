...
const deleteExpiredReservations = () => {
  const deleteQuery = `
    ...
    WHERE TIMESTAMPDIFF(HOUR, reservation_date, NOW()) >= 48;
  `;
...
app.get('/api/hotel/:HotelID/room/:roomid', (req, res) => {
  const roomId = req.params.roomid;
  const propretyId = req.params.HotelID;
  connection.query(
    'SELECT suite.*, reservations.start_date AS startDate, reservations.end_date AS endDate FROM suite JOIN reservations ON suite.RoomID = reservations.suiteid WHERE suite.RoomID = ? AND suite.HotelID = ?',
    [roomId, propretyId],
    (error, results) => {
      if (error) {
...
app.get('/api/hdzedfeotel/:HotelID/room/:roomid', (req, res) => {
  const roomId = req.params.roomid;
  const propretyID = req.params.HotelID;
  connection.query('SELECT * FROM suite WHERE RoomID = ? AND HotelID = ?', [roomId, propretyID], (error, results) => {
    if (error) {
      res.status(500).json({ error: 'Failed to retrieve proprety' });
    } else if (results.length === 0) {
      res.status(404).json({ error: 'User not proprety' });
...
app.post('/api/reservations', (req, res) => {
  const data = req.body;
  connection.query('INSERT INTO reservations SET ?', data, (error, results) => {
    if (error) {
      res.status(500).json({ error: 'Failed to add reservations' });
    } else {
      res.status(201).json({ message: 'reservation stats added successfully', id: results.insertId });
    }
  });
});
app.get('/api/reservations/:UserID', (req, res) => {
  const UserID = req.params.UserID;
  connection.query('SELECT * FROM reservations WHERE userid = ?', [UserID], (error, results) => {
    if (error) {
      res.status(500).json({ error: 'Failed to retrieve proprety rooms' });
    } else if (results.length === 0) {
      res.status(200).json([]); // Return an empty array if no rooms found
...
app.delete('/api/reservations/:id', (req, res) => {
  const userId = req.params.id;
  connection.query('DELETE FROM reservations WHERE id = ?', [userId], (error, results) => {
    if (error) {
...
// Add a new route to check for existing reservations for a specific user and room
app.get('/api/reservations/user/:userId/room/:roomId', (req, res) => {
  const userId = req.params.userId;
  const roomId = req.params.roomId;

  connection.query(
    'SELECT * FROM reservations WHERE userid = ? AND suiteid = ?',
    [userId, roomId],
    (error, results) => {
      if (error) {
